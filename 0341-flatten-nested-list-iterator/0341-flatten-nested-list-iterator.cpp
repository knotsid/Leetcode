	...
	*/
	bool hasNext() {        
        while(size(Start)){			
			...
			// First increment top of stack, so we know that all integers uptill(including top list) 
            // now have been traversed and then push the start iterator of top into stack, 
            // so that we traverse that one next
            Start.top()++;
