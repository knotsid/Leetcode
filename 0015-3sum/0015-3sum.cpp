...
public:
    vector<vector<int>> threeSum(vector<int>& nums) {
        ...
        //if given array has size less than 3 return null
        if(nums.size()< 3){
            return {};
        }
        //if 1st index is positive after sorting means sum can't be 0
        if(nums[0]>0){
            return {};
        }
        ...
        for(int i = 0 ; i < nums.size() ; ++i){     //Traversing the array to fix the number
            if(nums[i] > 0){     //If number fixed is +ve, stop there bcoz we need negative to make sum 0
                break;
            }
